...
import { simple_application, rich_text } from "monadic_react"
import * as BlobImage from "./blob_image";
...
export function Blob(
    ...
    mode:ViewUtils.EntityMode,
    src:string,
    delete_item:()=>Promise<void>,
    set_item:(src:File)=>Promise<void>) {
  return <BlobImage.BlobImage
      can_edit={can_edit && mode == "edit"}
      src={src}
      delete={() => delete_item()}
      upload={(new_src:File) => set_item(new_src)} />
}
...
export function RichText(
  ...
  set_item:(src:string)=>void) {
    let item = get_item()
    return simple_application<string>(rich_text(can_edit ? mode : "view")(item), (out) => can_edit? set_item(out) : null)
}
